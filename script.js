...
const BIN_ID = "6a6b2922f5f4af5e29d5199b"; // ex: 60abc123...
...
// ========== CARREGAR DADOS DO JSONBIN E INICIAR ==========
async function carregarDados() {
  try {
    ...
    CONFIG = dados.record.config;
    listaProdutos = dados.record.produtos.filter(p => p.ativo);
    inicializarSite();
  } catch (erro) {
    console.error("Erro ao carregar dados:", erro);
    // Fallback seguro se não carregar
    CONFIG = {
      horaAbertura: 7,
      horaFechamento: 23,
    ...
    listaProdutos = [];
    alert("Não foi possível carregar dados do servidor, usando configuração padrão.");
    inicializarSite();
  }
}

// ========== TODOS OS ELEMENTOS E FUNÇÕES ORIGINAIS AQUI ==========
function inicializarSite() {
  ...
  tipoAtendimentoEl?.addEventListener('change', () => {
    if (tipoAtendimentoEl.value === 'entrega') {
      ...
      taxaEntregaEl.value = (CONFIG.taxaEntregaPadrao || 8.00).toFixed(2).replace('.', ',');
    } else {
      campoTaxaEntregaEl.classList.add('oculto');
      blocoEnderecoEl.classList.add('oculto');
      limparCamposEndereco();
  ...
  // ======================
  // Produtos e Modal — agora carregados do JSONbin
  // ======================
  function carregarProdutosNaLista() {
    const container = document.querySelector('.lista-produtos');
    if (!container || !listaProdutos.length) return;
    container.innerHTML = "";

    listaProdutos.forEach(prod => {
      const prodEl = document.createElement('div');
      prodEl.className = "produto";
      prodEl.dataset.categoria = prod.categoria;
      prodEl.dataset.nome = prod.nome;
      prodEl.dataset.preco = prod.preco;
      prodEl.dataset.descricao = prod.descricao || "Sem descrição.";
      prodEl.dataset.imagem = prod.imagem || "https://via.placeholder.com/300";

      prodEl.innerHTML = `
        <div class="produto-imagem"><img src="${prod.imagem}" alt="${prod.nome}"></div>
    ...
    // Adiciona clique nos produtos carregados
  ...
  function atualizarCarrinho() {
    ...
    const taxa = usaTaxaEntrega ? (CONFIG.taxaEntregaPadrao || 8.00) : 0;
  ...
  document.getElementById('btn-finalizar')?.addEventListener('click', () => {
    ...
    const taxa = tipoAtendimento === 'entrega' ? (CONFIG.taxaEntregaPadrao || 8.00) : 0;
    ...
    let mensagem = `🍔 *PEDIDO #${numeroPedido} — ${CONFIG.nomeLoja || "Alison Burger"}*
📅 ${dataPedido}
👤 Cliente: ${nome}
...
💳 Forma de pagamento: ${pagamento}`;
    ...
    const urlWhatsApp = `https://wa.me/${CONFIG.numeroWhatsApp || "5519989021323"}?text=${encodeURIComponent(mensagem)}`;
    window.open(urlWhatsApp, '_blank');

    limparTudoCarrinho();
  });

  // Carrega produtos do painel na lista ao iniciar
  carregarProdutosNaLista();
}

// Inicia tudo carregando os dados
